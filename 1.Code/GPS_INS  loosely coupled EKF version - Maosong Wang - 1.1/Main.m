...
for i=1:data_length*200/2
    ...
    if(mod(i,10)==0)
        ...
        F(1:3,1:3)=-Vec2Skew(Winn);
        F(1:3,4:6)=Fev;
        F(1:3,7:9)=Fer;
        F(1:3,10:12)=-cbn;
        
        F(4:6,1:3)= Vec2Skew(Fn);
        F(4:6,4:6)=-Vec2Skew(TwoWien_Winn)+Vec2Skew(V_last)*Fev;
        F(4:6,7:9)= Vec2Skew(V_last)*DetaWr+Vec2Skew(V_last)*Fer;
        F(4:6,13:15)=cbn;
    ...
    if (mod(i,100)==0)
        ...
        q = QuatMult2(deta_q,q);
        %q = QuatMult(deta_q',q')'; %标准四元数乘法
        cbn=Qnb2Cbn(q);
        cnb = cbn';
        %%  把除了常值漂移之外的误差归零
        X(1:9,1)=zeros(9,1);
...
writetable(result_table,'Output/GnssInsResult.csv');
